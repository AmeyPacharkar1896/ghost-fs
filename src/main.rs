...
fn main() {
    ...
    // 1. Open the file
    let mut input_file = File::open("test.txt").expect("Please create a test.txt file in the same directory!");
    let mut buffer = Vec::new();
    input_file.read_to_end(&mut buffer).expect("Failed to read test.txt");
